...
function CaseModal({merchants,members,existing,close,saved}:{merchants:Merchant[];members:Member[];existing?:CaseItem|null;close:()=>void;saved:()=>void}) {
  ...
  async function submit(event:React.FormEvent){
    ...
    try{
      if(existing) await api(`/cases/${existing.id}`,{method:'PATCH',body:JSON.stringify({response:form.response,updateNote:form.updateNote,picUserId:form.picUserId,acrTicket:form.acrTicket,status:form.status})});
      else await api('/cases',{method:'POST',body:JSON.stringify(form)});
      saved();close();
  ...
  return <div className="overlay"><form className="modal case-modal" onSubmit={submit}>
    ...
    <label>Update<textarea rows={3} value={form.updateNote} onChange={event=>set('updateNote',event.target.value)} placeholder="Latest progress, follow-up, or blocker update"/></label>
...
export default function CasesPage({merchants,members}:{merchants:Merchant[];members:Member[]}) {
  ...
  function exportCases(){
    downloadCsv('case-report',['Merchant name','Merchant code','Issue','Category','Latest update','Response','PIC','PIC email','ACR ticket','Status','Created by','Created at','Last updated'],cases.map(item=>[
  ...
  return <main className="content cases-page">
    ...
    <section className="table-card">
      ...
      <table className="case-table"><thead><tr><th>Merchant / issue</th><th>Latest update</th><th>Category</th><th>PIC</th><th>ACR ticket</th><th>Status</th><th>Updated</th><th></th></tr></thead><tbody>{cases.map(item=><tr key={item.id}><td><div><b>{item.merchant.name}</b><small>{item.issue}</small>{item.response&&<small className="case-response">Response: {item.response}</small>}</div></td><td><span className="case-update">{item.updateNote||'No update yet'}</span></td><td>{categories[item.category]}</td><td>{item.pic.name}</td><td>{item.acrTicket||'—'}</td><td><span className={`case-status ${item.status.toLowerCase()}`}>{statuses[item.status]}</span></td><td>{new Date(item.updatedAt).toLocaleDateString('en-GB')}</td><td><button onClick={()=>setEditing(item)}>Update <ChevronRight/></button></td></tr>)}</tbody></table>
