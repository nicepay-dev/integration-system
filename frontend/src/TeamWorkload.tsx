...
export default function TeamWorkload({members,merchants,cases}:{members:Member[];merchants:Merchant[];cases:CaseItem[]}){
  ...
  const rows=useMemo(()=>{
    ...
    return members.map(member=>{
      ...
      return{...member,activeMerchants,openCases:memberCases.length,overdueCases,upcomingMeetings,total:activeMerchants+memberCases.length+upcomingMeetings};
  ...
  return <section className="team-workload">
    ...
    <div className="table-scroll"><table><thead><tr><th>Team member</th><th><Store/>Active merchants</th><th><BriefcaseBusiness/>Open cases</th><th><CircleAlert/>Overdue cases</th><th><CalendarClock/>Upcoming meetings</th></tr></thead>
      <tbody>{rows.map(row=><tr key={row.id}><td><b>{row.name}</b><small>{row.role}</small></td><td>{row.activeMerchants}</td><td>{row.openCases}</td><td><strong className={row.overdueCases?'workload-overdue':''}>{row.overdueCases}</strong></td><td>{row.upcomingMeetings}</td></tr>)}</tbody>
    </table></div>
  </section>;
}
