...
export class AuthService {
  ...
  async login(email:string,password:string,rememberMe=false){
    ...
    const expiresIn=rememberMe?this.config.get('JWT_REMEMBER_EXPIRES_IN','30d'):this.config.get('JWT_EXPIRES_IN','8h');
    const accessToken=await this.jwt.signAsync({sub:user.id,email:user.email,name:user.name,role:user.role},{secret:this.config.getOrThrow('JWT_SECRET'),expiresIn:expiresIn as any});
