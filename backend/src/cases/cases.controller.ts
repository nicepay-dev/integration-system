...
export class CasesController {
  ...
  @Get() list(@Query('status') status?:string,@Query('merchantId') merchantId?:string,@Query('picUserId') picUserId?:string,@Query('search') search?:string,@Query('dateFrom') dateFrom?:string,@Query('dateTo') dateTo?:string){return this.service.list(status,merchantId,picUserId,search,dateFrom,dateTo);}
